...
public abstract class ExtraHopperEntity extends LootableContainerBlockEntity implements Hopper {
    ...
    public ExtraHopperEntity(BlockEntityType<?> blockEntityType,BlockPos pos, BlockState state) {
    ...
    private static boolean insert(World world, BlockPos pos, BlockState state, Inventory inventory) {
        ...
        int amount = 1;
        if (be instanceof IronHopperEntity){
            amount = 2;
        } else if (be instanceof GoldHopperEntity){
            amount = 4;
        } else if (be instanceof DiamondHopperEntity){
            amount = 6;
            ...
            if (isInventoryFull(inventory2, direction)) {
                ...
                for(int i = 0; i < inventory.size(); ++i) {
                    if (!inventory.getStack(i).isEmpty()) {
                        ...
                        ItemStack itemStack2 = transfer(inventory, inventory2, inventory.removeStack(i, amount), direction);
